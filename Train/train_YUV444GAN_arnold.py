...
from Metric.metric_tool import iwssimgpu, IW_SSIM, LPIPS
from Metric.discriminator import HiFLCdiscriminator
...
class RateDistortionLoss(nn.Module):
    ...
    def __init__(self, args, distortion='mse', device="cuda", YUVweight = [1,1,1]):
        ...
        if 'LPIPS' in distortion:
            self.LPIPS_lossfn = LPIPS(net = 'alex', spatial = True).to(device)
    ...
    def forward(self, output, target, Test=False, Testi = torch.Tensor([0])):
        ...
        if not Test:
            ...
            out['g_loss'] = output["g_loss"] * self.lmbda_mse * 255 ** 2
            # obtain LPIPS loss
            out['LPIPS'] = self.LPIPS_lossfn(self.colorswith2(output["x_hat"]),
                                             self.colorswith2(target)).mean() * self.lmbda_mse * 255 ** 2
            out["distortion_loss"] = 5 * (
                        0.75 * out["distortion_loss"] + 0.00075 * out['g_loss'] + 0.005 * out['LPIPS']) / 6
        ...
        if Test:
            ...
            out['LPIPS'] = self.LPIPS_lossfn(rgb_hat.div(255), rgb_ori.div(255)).mean()
...
def train_one_epoch(model, criterion, train_dataloader, optimizer, aux_optimizer,
                    dis_model, optimizer_D, epoch, clip_max_norm, train_discriminator=True):
    model.train()
    ...
    for i, data in enumerate(train_dataloader):
        ...
        if train_discriminator:
            with torch.no_grad():
                out_net = model(yuvd)
            g_loss_pack, _, _ = dis_model([yuvd, out_net["x_hat"]], out_net["y_hat"])
            d_loss_real, d_loss_fake, g_loss = g_loss_pack
        ...
        out_net = model(yuvd)
        g_loss_pack, _, _ = dis_model([yuvd, out_net["x_hat"]], out_net["y_hat"])
        out_net["d_loss_real"], out_net["d_loss_fake"], out_net["g_loss"] = g_loss_pack
        ...
        if (i + 1) % 5000 == 0:
            print(
                ...
                f'd loss real loss: {out_net["d_loss_real"].item():.4f} |'
                f'd loss fake loss: {out_net["d_loss_fake"].item():.4f} |'
                f'g loss loss: {out_criterion["g_loss"].item():.4f} |'
                f'LPIPS loss: {out_criterion["LPIPS"].item():.4f} |'
                f'Aux loss: {aux_loss.item():.4f} |'
            )


def test_epoch(epoch, test_dataloader, model, criterion, dis_model=None):
    model.eval()
    model.update()
    device = next(model.parameters()).device
    ...
    with torch.no_grad():
        for data in test_dataloader:
            ...
            if dis_model:
                with torch.no_grad():
                    g_loss_pack, _, _ = dis_model([pad_d, out_net["x_hat"]], out_net["y_hat"])
            ...
            out_criterion["g_loss"] = gl

            aux_loss.update(model.aux_loss())
            bpp_loss.update(out_criterion["bpp_loss"])
            loss.update(out_criterion["loss"])
            psnr_loss.update(out_criterion["psnr"])
            g_losslog.update(out_criterion["g_loss"])
...
def main(argv):
    ...
    dis_model = HiFLCdiscriminator(Cy = 192)
    dis_model = dis_model.to(device)
    if args.cuda and torch.cuda.device_count() > 1:
        dis_model = CustomDataParallel(dis_model)
   # Initialize Discriminator optimizer
    optimizer_D = optim.Adam(filter(lambda p: p.requires_grad, dis_model.parameters()), lr = args.learning_rate)
    miles = [30]
    lr_scheduler_D = optim.lr_scheduler.MultiStepLR(optimizer_D, milestones = miles, gamma = 0.1)
    # Initialize Weights
    ...
    if len(Ckpt) != 0:
        ...
        _ = test_epoch(last_epoch - 1, test_dataloader, net, criterion, dis_model)
        print("Test results on {}:".format(test2path.split('/')[-1]))
        _ = test_epoch(last_epoch - 1, test_dataloader2, net, criterion, dis_model)
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        train_one_epoch(net, criterion, train_dataloader, optimizer, aux_optimizer,
                        dis_model, optimizer_D, epoch, args.clip_max_norm)
        print("Test results on {}:".format(testpath.split('/')[-1]))
        loss1 = test_epoch(epoch, test_dataloader, net, criterion, dis_model)
        print("Test results on {}:".format(test2path.split('/')[-1]))
        loss2 = test_epoch(epoch, test_dataloader2, net, criterion, dis_model)
